...
- **Безопасность**
  ...
  - Circuit breaker для защиты БД от перегрузки при обновлении `last_seen_at`.
...
- **WebSocket метрики**:
  ...
  - `chat_websocket_messages_total{message_type}` — количество сообщений WebSocket по типам (`message`, `ephemeral_key`, `session_established`, `ack`, `file_start`, `file_chunk`, `file_complete`, `typing`, `reaction`, `message_delete`);
...
- **Circuit breaker метрики**:
  ...
  - `circuit_breaker_failures_total{name}` — количество failures в circuit breaker (используется для обновления `last_seen_at`).

### Интеграция с Prometheus

Метрики готовы для сбора Prometheus. Пример конфигурации `prometheus.yml`:
...
- Добавить предпросмотр файлов/при отправке файлов спрашивать разрешить ли скачивание или только просмотр (тогда в UI отображать скачать/посмотреть или только посмотреть).
- Отображение fingerprint подтверждения до создания соединения и если оба подтвердили, то тогда пытаться создать защищенное соединение. (добавить состояние "ожидание верификации" и изменить порядок запросов).
- При протухании токена (15 мин) выводится сообщение "не удалось найти приватный ключ" (WebSocket переподключается со старым токеном).
- Настройка инфраструктуры мониторинга: развертывание Prometheus для сбора метрик и Grafana для визуализации. Метрики уже экспортируются в формате Prometheus через `/metrics` endpoint на каждом сервисе, требуется только настроить сбор и дашборды.
