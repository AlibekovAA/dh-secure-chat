...
- **Отправка файлов**
  ...
  - модальное окно предпросмотра файлов с поддержкой изображений, PDF и текстовых файлов;
...
Prometheus уже настроен и готов к использованию. Метрики собираются автоматически с обоих сервисов.

#### Запуск Prometheus

Prometheus включен в `docker-compose.yml` и запускается вместе с остальными сервисами:

```bash
docker-compose up -d prometheus
```

После запуска Prometheus UI доступен по адресу: **http://localhost:9090**
...
4. В разделе **Graph** можно выполнять запросы к метрикам, например:
...
- SPA с основными модулями:
  ...
  - модальное окно предпросмотра файлов (изображения, PDF, текстовые файлы);
...
- **Мониторинг (Prometheus)**
  - Автоматический сбор метрик с auth-service и chat-service.
  - Web UI доступен на порту 9090.
...
- Настройка Grafana для визуализации метрик: создание дашбордов для отображения метрик, собранных Prometheus.
