...
- **WebSocket метрики**:
  ...
  - `chat_websocket_messages_total{message_type}` — количество сообщений WebSocket по типам (`message`, `ephemeral_key`, `session_established`, `ack`, `file_start`, `file_chunk`, `file_complete`, `typing`, `reaction`, `message_delete`, `message_edit`);
