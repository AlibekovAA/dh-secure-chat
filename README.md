...
### Fingerprint‑верификация (планируется)

- Для каждого собеседника будет отображаться **fingerprint** его публичных ключей (identity/ephemeral).
- Пользователи смогут сверить fingerprint по другому каналу связи.
- При расхождении fingerprint UI будет явно предупреждать о возможной атаке «man‑in‑the‑middle».
...
- SPA с основными модулями:
  ...
  - экран/модальное окно для fingerprint‑верификации (планируется).
